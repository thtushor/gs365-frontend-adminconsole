...
import CreateAmbassador from "./CreateAmbassador";
...
const Ambassador = () => {
  ...
  const [ambassadorToEdit, setAmbassadorToEdit] = useState(null);
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () =>
      getRequest({
        url: BASE_URL + API_LIST.GET_AMBASSADOR,
        params: filters,
        errorMessage: "Failed to fetch ambassador list",
      }),
  ...
  const handleEditClick = (announcement) => {
    setAmbassadorToEdit(announcement);
  ...
  return (
    <div className="w-full p-5 bg-white rounded-xl border border-green-300">
      <CreateAmbassador
        ambassadorToEdit={ambassadorToEdit}
        onSuccess={handleSuccess}
        onCancel={() => setAmbassadorToEdit(null)}
      />
      ...
      <div className="mt-6 border border-gray-300 p-5 rounded-lg">
        ...
        {isLoading ? (
          ...
          <>
            <DataTable columns={columns} data={ambassadors} />
            <Pagination
      ...
      <BaseModal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
        {ambassadorToDelete && (
          <DeleteConfirmationPopup
            apiUrl={`${BASE_URL}${API_LIST.DELETE_AMBASSADOR}/${ambassadorToDelete.id}`}
            payload={{}}
            onSuccess={() => {
              queryClient.invalidateQueries({ queryKey: ["ambassadors"] });
            ...
            message={`Are you sure you want to delete the ambassador "${ambassadorToDelete.title}"?`}
          />
        )}
      </BaseModal>
    </div>
