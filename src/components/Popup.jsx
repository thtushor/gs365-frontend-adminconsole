...
const Popup = () => {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["popup", filters],
    queryFn: () =>
      getRequest({
        url: BASE_URL + API_LIST.GET_POPUP,
        params: filters,
        errorMessage: "Failed to fetch popups list",
      }),
  ...
  return (
    <div className="w-full p-5 bg-white rounded-xl border border-green-300">
      <CreatePopup
        ...
        onCancel={() => setPopupToEdit(null)}
        popupToEdit={popupToEdit}
      />

      <div className="mt-6 border border-gray-300 p-5 rounded-lg">
        <h2 className="text-[20px] font-semibold mb-4 text-left">Popup List</h2>
        ...
        {isLoading ? (
          ...
          <p className="text-center text-gray-500">No popup found.</p>
        ) : (
          <>
            <DataTable columns={columns} data={popup} />
            <Pagination
      ...
      <BaseModal open={isModalOpen} onClose={() => setIsModalOpen(false)}>
        {popupToDelete && (
          <DeleteConfirmationPopup
            apiUrl={`${BASE_URL}${API_LIST.DELETE_POPUP}/${popupToDelete.id}`}
            payload={{}} // optional
            onSuccess={() => {
              queryClient.invalidateQueries({ queryKey: ["popup"] });
            ...
            message={`Are you sure you want to delete the popup "${popupToDelete.title}"?`}
          />
        )}
      </BaseModal>
    </div>
