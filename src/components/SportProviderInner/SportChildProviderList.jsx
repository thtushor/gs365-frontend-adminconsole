...
const SportChildProviderList = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    name: "",
    status: "",
    parentId: sportProviderId,
  });

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["sport_provider", filters],
    queryFn: () =>
      getRequest({
        url: BASE_URL + API_LIST.GET_SPORT_PROVIDER,
        params: filters,
        errorMessage: "Failed to fetch sport provider list",
      }),
    keepPreviousData: true,
  });

  const sport_provider = data?.data || [];
    ...
    <div className="bg-white rounded-lg shadow p-4 mt-6 w-full">
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          onClick={() =>
            navigate(
              `/add-sport-provider?ref_parent_id=${sportProviderInfo?.id}`
            )
          }
        >
          + Sport Provider
      ...
      {isLoading ? (
        <div className="text-center text-gray-500 py-8">
          Loading sport providers...
        </div>
      ) : isError ? (
        <div className="text-center text-red-500 py-8">
          Failed to load sport providers: {error?.message || "Unknown error"}
        </div>
      ) : sport_provider.length === 0 ? (
        <div className="text-center text-gray-500 py-8">
          No sport providers found.
        </div>
      ) : (
        <>
          <DataTable columns={columns} data={sport_provider} />
          <Pagination
...
export default SportChildProviderList;
