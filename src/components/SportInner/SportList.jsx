...
const SportList = ({ providerId }) => {
  ...
  const columns = [
      ...
      render: (_, row) =>
        row.sportUrl ? (
          <button
            onClick={() => {
              navigator.clipboard
                .writeText(row.sportUrl)
                .then(() => toast.success("Sport URL copied!"))
    ...
    },
    {
      ...
      render: (_, row) =>
        row.providerInfo?.id ? (
          <Link
            to={`/sport-provider-list/${row.providerInfo?.id}`}
  ...
  return (
    ...
    >
      ...
      {/* Table */}
      {isLoading ? (
        <div className="text-center text-gray-500 py-8">Loading sports...</div>
      ) : isError ? (
