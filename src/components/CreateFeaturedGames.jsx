...
const CreateFeaturedGames = ({ bannerToEdit, handleCancelEdit, onSuccess }) => {
  ...
  useEffect(() => {
    if (bannerToEdit) {
      ...
      if (bannerToEdit.gameId) {
        const preSelected = sportsOption.find(
          (opt) => opt.value === bannerToEdit.gameId
        );
        setSelectedGame(preSelected || null);
      } else {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const url = BASE_URL + API_LIST.CREATE_FEATURED_GAME;
  ...
  return (
    <div>
      <h2 className="text-[20px] font-semibold mb-4 text-left">
        {bannerToEdit ? "Update Featured Game" : "Create Featured Game"}
      </h2>
...
export default CreateFeaturedGames;
